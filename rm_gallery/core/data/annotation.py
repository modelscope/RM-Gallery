...
class DataAnnotation(BaseDataModule):
    ...
    label_config: str = Field(..., description="Label Studio labeling configuration")
    ...
    def export_annotations_to_dataset(
        self, filename: Optional[str] = None, include_original_data: bool = True
    ) -> BaseDataSet:
        ...
        if filename:
            ...
            for sample in annotated_samples:
                export_data.append(sample.dict())

            with open(filename, "w") as f:
                json.dump(export_data, f, indent=2, default=str)
            logger.info(f"Saved annotated DataSamples to: {filename}")
        ...
        return BaseDataSet(
            ...
            metadata={
                ...
                "annotation_export_file": filename,
    ...
    def _convert_annotations_to_schema(
        ...
        for annotation in annotations:
            ...
            if self.export_processor:
                try:
                    processor_func = get_export_processor(self.export_processor)
                    if processor_func:
                        processed_data = processor_func(annotation_data)
...
def create_annotation_module(
    ...
    annotation_module = DataAnnotation(
        name=name,
        label_config=label_config,
        project_title=project_title,
        project_description=project_description,
